...
options.layers = L.tileLayer.grayscale( options.style, {
...
function onEachFeature(feature, layer) {
	...
	if (feature.properties) {
		...
		var latValue = geometry.coordinates[1];
		var lngValue = geometry.coordinates[0];
		//var location = "";
	    popUpContent += "<h5>Parada: #"+unit+"</h5>";
      	popUpContent += "<ul>";
      	popUpContent += "<li class='location'></li>";
      	popUpContent += "<li class='img'></li>";
      	popUpContent += "</ul>";
		layer.bindPopup(popUpContent, customOptions);
...
function getLocationFromGoogle(lat,lng){
	var location = "";
...
$("#rerutas").click(function (e) {
	...
	$(".container-listado-rutas ul li").removeClass( "colorActivo" ).addClass( "colorDesactivo" );
	$(".container-listado-rutas ul li .img-full").hide();
	$(".container-listado-rutas ul li .info-ruta").hide();
});
...
$(".item-route").click(function (e) {
	// show loading
	$( ".container-listado-rutas ul li" ).trigger( "click" );
	...
	var folder = $(this).attr('data-src');
	var routeId = $(this).attr('id');
	var tipo = $(this).attr('data-tipo');
	var subFolderRoute = "";
	var subFolderStops = "";

	if(tipo == "circuito" || tipo == "ruta"){
		subFolderRoute = "route";
		subFolderStops = "stops";
	}
	else if(tipo == "ida" || tipo == "vuelta"){
		subFolderRoute = tipo+"/route";
		subFolderStops = tipo+"/stops";
	}

	var genericLocation = "/rutas/data/"+folder;
	var geojsonRouteFile = genericLocation+"/"+subFolderRoute+"/route.json";
	var geojsonStopFile = genericLocation+"/"+subFolderStops+"/route_"+routeId+"_stops.geojson";
    ...
    map.on('popupopen', function (e) {
    	...
    	var _latlng = e.popup._latlng;
    	var _content = e.popup._content;
    	var _contentNode = e.popup._contentNode;
    	var latlng = { lat: parseFloat(_latlng.lat), lng: parseFloat(_latlng.lng) };
    	var location = "";
    	var geocoder = new google.maps.Geocoder;
		geocoder.geocode({'location': latlng}, function(results, status) {
			if (status === 'OK') {
	        ...
	        var li = $(_contentNode).find("li.location");
	        li.html("Ubicación: "+location+"");
	        // find image
	        var liImgStop = $(_contentNode).find("li.img");
	        var mapillaryApi = "https://a.mapillary.com/v3/images?"+
	        "client_id=OXNNd3MyeWlFaThqdmtlVUdVSk9nUTo0Mzg4YjJiZjUyYTJiYzc0&closeto="+parseFloat(_latlng.lng)+","+parseFloat(_latlng.lat)+"";
	        $.ajax({
	        	url: mapillaryApi,
	        	dataType: "json",
	        	success: function (data){
	        		var geoJson = data;
		          	if(geoJson.features.length > 0){
		          		// get the first image
		          		var imgKey = geoJson.features[0].properties.key;
		          		var imgUrl = "https://d1cuyjsrcm0gby.cloudfront.net/"+imgKey+"/thumb-320.jpg";
		          		var img = "<img src='"+imgUrl+"'/>";
		          		liImgStop.html(img);
		          		map.setView([parseFloat(_latlng.lat),parseFloat(_latlng.lng)],14);
		          	}
		          	else{
		          		liImgStop.html("Imagen no disponible:");
		          	}
		          	$('.layer-map').removeClass('show');
	          },
	          error: function (xhr) {
	          	liImgStop.html("Error:"+ xhr.statusText);
	          	map.setView([parseFloat(_latlng.lat),parseFloat(_latlng.lng)],14);
	          	$('.layer-map').removeClass('show');
	          }
	        });
	...
	setTimeout(function()
	{
		var geojsonRoute = new L.GeoJSON.AJAX([geojsonRouteFile],{style:myStyle}).addTo(map);

		var geojsonStops = new L.GeoJSON.AJAX([geojsonStopFile],{onEachFeature:onEachFeature}).addTo(map);
